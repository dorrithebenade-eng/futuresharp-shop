// Personeel-beskermd — skrap 'n inskrywing uit die "sprekers"-store.
// FutureSharp Talks: gekloon uit skrap-outeur.js.
// Blokkeer NIE skrapping as dit reeds op 'n talk se verdeling gebruik
...
//
// 'n Spreker wat deur die uitnodigingsvloei gekom het, dra `dokumente`: sy
// ID-afskrif en die ondertekende ooreenkoms. (Die sprekersuitnodiging kom
// eers wanneer die sprekersooreenkoms bestaan; die kode is nou reeds reg.) Daardie lêers word NOOIT na die
// sprekersrekord gekopieer nie — hulle bly in `uitnodiging-leers` en die rekord
...
exports.handler = async (event, context) => {
  ...
  const spreker_id = (invoer.spreker_id || "").trim();
  if (!spreker_id) {
    return { statusCode: 400, body: "Verpligte veld: spreker_id" };
  }

  const store = kry_store("sprekers");

  const bestaande = await store.get(spreker_id, { type: "json" });
  if (!bestaande) {
    return { statusCode: 404, body: `Geen inskrywing met ID "${spreker_id}" gevind nie` };
  }
  ...
  for (const soort of Object.keys(dokumente)) {
    ...
    try {
      ...
      // mens die spreker nooit kon verwyder nie. Die antwoord se watter een
      // agtergebly het, sodat dit met die hand nagegaan kan word.
      console.error(
        `Kon nie ${soort} (${dok.store}/${dok.sleutel}) vir spreker ${spreker_id} skrap nie:`,
        fout
      );
  ...
  await store.delete(spreker_id);

  if (leers_misluk.length) {
    console.warn(
      `Spreker ${spreker_id} is geskrap, maar hierdie leers het agtergebly: ${leers_misluk.join(", ")}`
    );
  }
